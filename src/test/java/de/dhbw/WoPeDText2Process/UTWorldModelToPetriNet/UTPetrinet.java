...
public class UTPetrinet extends T2PUnitTest {
  ...
  public void evaluatePetriNet() {
    assertEquals(
        "Referencing arcs for Elements are not extracted correctly.",
        true,
        evluateGetReferencingArcs());
    assertEquals(
        "Workflownet Transformation was applied incorrectly.",
        true,
        evaluateTransformToWorkflowNet());
  }
  ...
  private boolean evluateGetReferencingArcs() {
    ...
    boolean all =
        pn.getAllReferencingArcsForElement(transitions[0].getID(), PetriNet.REFERENCE_DIRECTION.all)
                .size()
            == 4;
    boolean ingoing =
        pn.getAllReferencingArcsForElement(
                    transitions[0].getID(), PetriNet.REFERENCE_DIRECTION.ingoing)
                .size()
            == 3;
    boolean outgoing =
        pn.getAllReferencingArcsForElement(
                    transitions[0].getID(), PetriNet.REFERENCE_DIRECTION.outgoing)
                .size()
            == 1;
    boolean none =
        pn.getAllReferencingArcsForElement(transitions[1].getID(), PetriNet.REFERENCE_DIRECTION.all)
                .size()
            == 0;
  ...
  private boolean evaluateTransformToWorkflowNet() {
    ...
    while (j.hasNext()) {
      Transition t = j.next();
      if (pn.getAllReferencingArcsForElement(t.getID(), PetriNet.REFERENCE_DIRECTION.ingoing).size()
              == 0
          || pn.getAllReferencingArcsForElement(t.getID(), PetriNet.REFERENCE_DIRECTION.outgoing)
                  .size()
              == 0) {
    ...
    while (i.hasNext()) {
      Place p = i.next();
      if (pn.getAllReferencingArcsForElement(p.getID(), PetriNet.REFERENCE_DIRECTION.ingoing).size()
          == 0) {
        sourceCount++;
      }
      if (pn.getAllReferencingArcsForElement(p.getID(), PetriNet.REFERENCE_DIRECTION.outgoing)
              .size()
          == 0) {
        sinkCount++;
      }
    ...
    // Reapply the logic to Petrinet enhanced by Workflownet Properties, to see if it remains
    // unchanged -> check via Metadata
    try {
      pn.transformToWorkflowNet();
    } catch (PetrinetGenerationException e) {
      e.printStackTrace();
