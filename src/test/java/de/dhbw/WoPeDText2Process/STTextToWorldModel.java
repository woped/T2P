...
public class STTextToWorldModel extends T2PScenarioTest {
  ...
  private static String[] TestExamples = {
    ...
    "ST_Resource_Bike_Manufacturing.xml",
  ...
  public void evaluateWorldModelBuild() throws IOException {
    ...
    for (int i = 0; i < TestExamples.length; i++) {
      ...
      assertEquals(
          "Generated WorldModel for Testexample "
              + (i + 1)
              + " "
              + TestExamples[i]
  ...
  private void printComparison(
      List<? extends SpecifiedElement> elements, ArrayList<String> testElements) {
  ...
  private static double calculateResourceDelta(WorldModel wm) {
    ...
    return calculateDeltaScore(
        resourceCount, getWorldModelElementTypeCount(ELEMENT_TYPE_RESOURCES));
  }
  ...
  public ArrayList<String> getWorldModelElementList(String[] elementType) {
    ...
    for (int i = 0; i < elementCount; i++) {
      try {
        String text =
            xp.evaluate(
                "//" + elementType[0] + "/" + elementType[1] + "[" + (i + 1) + "]/Name",
