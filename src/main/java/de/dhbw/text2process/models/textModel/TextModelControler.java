...
public class TextModelControler extends ProcessUtils {
  ...
  private void highlightSpecifiedElement(
      SentenceNode _sn, SpecifiedElement a, boolean highlightDependants) {
    ProcessNode _pn;
  ...
  /**
   * searches for an object with the given string in the given sentence index is a startPosition
   *
   * @param _sn
   * @param determiner
   * @param wordIndex
   * @return
   */
  private ProcessNode getElement(SentenceNode sn, String name, int index) {
    if (name != null) {
  ...
  public void setModels(
      TextToProcess processor, TextAnalyzer analyzer, TextModelBuilder builder, TextModel model) {
    f_analyzer = analyzer;
    f_builder = builder;
    f_model = model;
