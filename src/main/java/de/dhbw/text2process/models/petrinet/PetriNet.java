...
public class PetriNet {
  ...
  Logger logger = LoggerFactory.getLogger(PetriNet.class);

  public enum REFERENCE_DIRECTION {
    ingoing,
    outgoing,
    all
  };

  public enum ISOLATED_PLACE_TYPE {
    source,
    sink
  };
  ...
  public void transformToWorkflowNet() throws PetrinetGenerationException {
    if (isEmptyPetrinet()) {
      throw new PetrinetGenerationException(
          "Workflownet cant be created based on the given input.");
    }
  ...
  public String getPNML() {
    logger.debug("Creating the PNML xml node");
    String PNML =
    ...
    logger.debug("Appending the places to the pnml object");
    Iterator<Place> i = placeList.iterator();
    while (i.hasNext()) {
      Place p = i.next();
      // TODO: check if the new design works
      PNML = PNML + p.getXmlString() + "\n";
    }

    logger.debug("Appending the transitions to the pnml object");
    ...
    logger.debug("Appending the arc to the pnml object");
    Iterator<Arc> i3 = arcList.iterator();
    while (i3.hasNext()) {
      Arc a = i3.next();
      // TODO: check if the new design works
      PNML = PNML + a.getXmlString() + "\n";
    }

    logger.debug("Finishing the pnml xml");
    PNML =
        PNML
  ...
  public PetriNetElement getPetrinetElementByID(String ID) {
    ArrayList<ArrayList<? extends PetriNetElement>> elementLists =
        new ArrayList<ArrayList<? extends PetriNetElement>>();
  ...
  public List<Arc> getAllReferencingArcsForElement(
      String ID, REFERENCE_DIRECTION directionRestriction) {
    ...
    if (element != null) {
      ...
      while (i.hasNext()) {
        ...
        switch (directionRestriction) {
          case ingoing:
            if (a.getTarget().equals(element.getID())) referencingArcs.add(a);
            break;
          case outgoing:
            if (a.getSource().equals(element.getID())) referencingArcs.add(a);
            break;
          default:
  ...
  private List<Place> getAllIsolatedElements(ISOLATED_PLACE_TYPE placeType) {
    ...
    while (i.hasNext()) {
      ...
      switch (placeType) {
        case sink:
          if (isSink(p)) isolatedPlaces.add(p);
          break;
        case source:
          if (isSource(p)) isolatedPlaces.add(p);
          break;
        default:
          break;
      }
  ...
  private void healDanglingTransitions() {
    ...
    while (i.hasNext()) {
      ...
      boolean hasOutgoingArcs =
          getAllReferencingArcsForElement(t.getID(), REFERENCE_DIRECTION.outgoing).size() > 0;
      boolean hasIngoingArcs =
          getAllReferencingArcsForElement(t.getID(), REFERENCE_DIRECTION.ingoing).size() > 0;
