...
/**
 * wraps all of the functionality to create processes from text. Load and analyze a text using
 * "parseText". To reanalyze a text simple use "analyzeText".
 */
public class TextToProcess {
  ...
  /**
   * Sets the element map which comes from the ProcessModelBuilder and can be used to map actions to
   * task nodes etc.
   *
   * @param map
   */
  ...
  public void textModelElementClicked(ProcessObject o) {
    if (o instanceof SentenceNode) {
      ...
      if (_sentence != null) {
        ...
        Collection<TypedDependency> _list =
            _sentence.getGrammaticalStructure().typedDependenciesCollapsed();
      ...
      if (f_elementsMapInv.containsKey(o)) {
        ...
        if (f_textModelControler != null) f_textModelControler.highlightAction(_ac);
      }
    }
  }
   ...
   */
  public void addManualReferenceResolution(
      SentenceWordID sentenceWordID, SentenceWordID sentenceWordID2) {
