...
public class T2PController {
	 ...
	 */
	@ApiOperation(value = "Translate text into a process model type bpmn.")
	@PostMapping(value = "/generateBPMNv2", consumes = "application/json", produces = "application/json")
	public String generateBPMNFromTextv2(@RequestBody String param, HttpServletRequest request,
									   HttpServletResponse response) {
	 ...
	 *
	 * <p>
	 * POST based controller method. Tailored to generate a PetriNet from a given
	 * Text. Use the proper URL to get generate the String which can be interpreted
	 * by the WoPeD-Tool.
	 * </p>
	 *
	 * @author <a href="mailto:kanzler.benjamin@student.dhbe-karlsruhe.de">Benjamin
	...
	public String generatePetriNetFromText(@RequestBody String param, HttpServletRequest request,
		...
		try {
			...
			logger.error(
					"The PetriNet was not properly built. Please contact the development department. <a href=mailto:woped-service@dhbw-karlsruhe.de>WoPeD-Service<a>");
			logger.error(e.getMessage());
			...
			logger.error(
					"An unexpected error interrupted the process. Exception information is given to the return json");
			logger.error(e.getMessage());
			response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
			pnmlResponse = new Response<String>(true, e.getCause(), e.getMessage(), e.getStackTrace());
		}
