...
public class ElementsBuilder {
  ...
  public static Actor createActor(
    ...
    if (!wnf.canBePersonOrSystem(_fullNoun, node.value().toLowerCase())) {
      ...
      if (node.parent().value().equals("CD")
          || wnf.canBeGroupAction(node.value())) { // one of the physicians
        List<TypedDependency> _preps = SearchUtils.findDependency("prep", dependencies);
        for (TypedDependency spec : _preps) {
          if (Constants.f_realActorPPIndicators.contains(spec.reln().getSpecific())
              && spec.gov().equals(node)) {
  ...
  private static Actor createActorInternal(
      T2PSentence origin,
      List<Tree> fullSentence,
      TreeGraphNode node,
      Collection<TypedDependency> dependencies) {
  ...
  public static Action createAction(
    ...
    if (_aux.length() > 0) _result.setAux(_aux);
    ...
    List<TypedDependency> _toCheck =
        SearchUtils.findDependency(ListUtils.getList("xcomp", "dep"), dependencies);
...
//  }
   ...
   * flag isMessageTriggered in Transition already added. TriggerType is implemented in the Action
   * class.
   *
   * @param dependencies
   * @param wnf
   ...
   * @return
   */
  private boolean checkIsMessageTriggered(
      Collection<TypedDependency> dependencies, WordNetFunctionality wnf, TreeGraphNode node) {
    // wnf.getListHypernym()
    return true;
  }
  ...
  private static void checkDobj(
    ...
    for (TypedDependency td : _toCheck) {
      if (td.gov().equals(node)) {
        ...
        Specifier _sp =
            new Specifier(origin, td.dep().index(), getFullNoun(td.dep(), dependencies));
        _sp.setSpecifierType(SpecifierType.DOBJ);
        ExtractedObject _obj =
            ElementsBuilder.createObject(origin, fullSentence, td.dep(), dependencies);
        _sp.setObject(_obj);
   ...
   * @return
   */
  private static TreeGraphNode getCop(
      TreeGraphNode node, Collection<TypedDependency> dependencies) {
    List<TypedDependency> _toCheck =
        SearchUtils.findDependency(ListUtils.getList("cop"), dependencies);
    for (TypedDependency td : _toCheck) {
   ...
   * @return
   */
  private static TreeGraphNode getIObj(
      TreeGraphNode node, Collection<TypedDependency> dependencies) {
    List<TypedDependency> _toCheck =
        SearchUtils.findDependency(ListUtils.getList("iobj"), dependencies);
    for (TypedDependency td : _toCheck) {
      if (td.dep().equals(node)) {
        // found something
        return td.gov();
  ...
  /**
   * cop(case-6, is-3) neg(case-6, not-4)
   *
   * @param node
   * @param dependencies
   * @return
  ...
  private static String getAuxiliaries(
      TreeGraphNode node, Collection<TypedDependency> dependencies) {
    List<String> _lookFor = ListUtils.getList("aux", "auxpass");
    return findDependants(node, dependencies, _lookFor, true);
  }

  private static String findDependants(
      TreeGraphNode node,
      Collection<TypedDependency> dependencies,
      List<String> lookFor,
      boolean isGovernor) {
  ...
  private static TreeGraphNode getModifiers(
      TreeGraphNode node, Collection<TypedDependency> dependencies) {
    List<TypedDependency> _toCheck =
        SearchUtils.findDependency(ListUtils.getList("advmod", "acomp"), dependencies);
    for (TypedDependency td : _toCheck) {
      if (td.gov().equals(node)) {
        // only take following modifiers as other are adverbs (e.g. quickly walk vs. walk >out<)
        if (td.gov().index() < td.dep().index()
            && !Constants.f_sequenceIndicators.contains(td.dep().value())) {
          // found something
          return td.dep();
        }
      }
  ...
  public static Action createActionSyntax(
      T2PSentence origin, List<Tree> fullSentence, Tree vpHead, boolean active) {
  ...
  private static List<Tree> extractVerbParts(Tree node, boolean active) {
    ...
    if ((node.isLeaf())) {
      ...
      ||
      node.label().value().equals("PP")) { */
      // Indicates some Verb Form
      _result.add(node);
    } else {
  ...
  /**
   * creates a new specified elements which can either be a Resource or an Actor
   *
   * @param origin
   * @param world
   * @param fullSentence
   ...
   * @return
   */
  public static ExtractedObject createObject(
      T2PSentence origin,
      List<Tree> fullSentence,
      TreeGraphNode node,
      Collection<TypedDependency> dependencies) {
    ...
    WordNetFunctionality wnf = new WordNetFunctionality();
    if (wnf.canBePersonOrSystem(_fullNoun, node.value().toLowerCase())
        || ProcessingUtils.canBePersonPronoun(node.value())) {
  ...
  private static String getFullNoun(TreeGraphNode node, Collection<TypedDependency> dependencies) {
    List<TypedDependency> _toCheck =
        SearchUtils.findDependency(ListUtils.getList("nn", "dep"), dependencies);
  ...
  private static void determineNounSpecifiers(
      T2PSentence origin,
      List<Tree> fullSentence,
      TreeGraphNode node,
      Collection<TypedDependency> dependencies,
   ...
   * @param element
   */
  private static void findINFMODSpecifiers(
      T2PSentence origin,
      TreeGraphNode node,
      Collection<TypedDependency> dependencies,
    ...
    if (_toCheck.size() > 0) {
      ...
      for (TypedDependency td : _toCheck) {
        if (td.gov().equals(node)) {
          // found it
          _toCheck =
              SearchUtils.findDependency(ListUtils.getList("aux", "cop", "neg"), dependencies);
   ...
   * @param element
   */
  private static void getPARTMODSpecifiers(
      T2PSentence origin,
      TreeGraphNode node,
      Collection<TypedDependency> dependencies,
      ExtractedObject element) {
  ...
  private static void extractPPSpecifierSyntax(
      T2PSentence origin, List<Tree> fullSentence, SpecifiedElement element, Tree fullPhrase) {
    // search for a PP determiner
    List<Tree> _ppList = getPPSpecifierSyntax(fullPhrase);
    for (Tree pp : _ppList) {
      Specifier _sp =
          new Specifier(
              origin, SearchUtils.getIndex(fullSentence, _ppList), PrintUtils.toString(pp));
      _sp.setSpecifierType(SpecifierType.PP);
      element.addSpecifiers(_sp);
    }
  }

  private static void extractPPSpecifier(
      T2PSentence origin,
      List<Tree> fullSentence,
      SpecifiedElement element,
      TreeGraphNode node,
    ...
    // search for a PP determiner
    List<TypedDependency> _toCheck =
        SearchUtils.findDependency(ListUtils.getList("prep", "prepc"), dependencies);
    List<TypedDependency> _rcMod =
        SearchUtils.findDependency(ListUtils.getList("rcmod"), dependencies);
    for (TypedDependency td : _toCheck) {
      String _cop = null;
      ...
      if ((td.gov().equals(node) || td.gov().value().equals(_cop)) && !partOfrcMod(_rcMod, td)) {
        ...
        if (!_phraseTree.parent().value().equals("PRN")) {
          ...
          if (_phrase.indexOf(' ') >= 0) { // does not have to S(interact ...)
            if (td.reln().getSpecific()
                != null) { // ... is reviewed, resulting in.... (BPMN MRG Ex5)
              _phrase =
                  _phrase.substring(
                      _phrase.indexOf(' ')); // cutting of first word (is included in getSpecific)
  ...
  private static void extractRCMODSpecifier(
      T2PSentence origin,
      SpecifiedElement element,
      TreeGraphNode node,
      Collection<TypedDependency> dependencies) {
  ...
  private static void extractSBARSpecifier(
      T2PSentence origin,
      List<Tree> fullSentence,
      SpecifiedElement element,
      Tree phraseHead,
    ...
    for (Tree sbar : _sbarList) {
      ...
      if (idx1 > idx2) {
        Specifier _sp =
            new Specifier(
                origin,
                SearchUtils.getIndex(fullSentence, sbar.getLeaves()),
                PrintUtils.toString(sbar));
   ...
   * @param _a
   */
  private static void findAMODSpecifiers(
      T2PSentence origin,
      TreeGraphNode node,
      Collection<TypedDependency> dependencies,
      SpecifiedElement element) {
  ...
  private static void findNNSpecifiers(
      T2PSentence origin,
      TreeGraphNode node,
      Collection<TypedDependency> dependencies,
      SpecifiedElement element) {
  ...
  private static void getSpecifierFromDependencies(
      T2PSentence origin,
      TreeGraphNode node,
      Collection<TypedDependency> dependencies,
      SpecifiedElement element,
  ...
  private static void findDeterminer(
      TreeGraphNode node, Collection<TypedDependency> dependencies, ExtractedObject _r) {
