...
public class SearchUtils {
   ...
   * @return
   */
  public static List<TypedDependency> findDependency(
      String relName, Collection<TypedDependency> dependencies) {
   ...
   */
  public static List<TypedDependency> findDependency(
      List<String> relNames, Collection<TypedDependency> dependencies) {
    List<TypedDependency> _result = new ArrayList<TypedDependency>();
  ...
  /**
   * find the first occurrence of the given tag in a tree. depth-first search from left to right
   *
   * @param lookFor
   * @param t
   * @return
  ...
  /**
   * returns the actions from the list of actions which contains the specified element in some way
   * (actor, resource, xcomp...)
   *
   * @param to
   * @param actions
   * @return
  ...
  public static Action getAction(SpecifiedElement to, List<Action> actions) {
    ...
    for (Action a : actions) {
      ...
      if (a.getActorFrom() != null
          && (to.equals(a.getActorFrom())
              || specifiersContain(a.getActorFrom().getSpecifiers(), to))) {
        return a;
      }
      if (a.getObject() != null
          && (to.equals(a.getObject()) || specifiersContain(a.getObject().getSpecifiers(), to))) {
        return a;
      }
      if (a.getXcomp() != null) {
        if (specifiersContain(a.getXcomp().getSpecifiers(), to)) {
          return a;
            ...
            && (to.equals(a.getXcomp().getObject())
                || specifiersContain(a.getXcomp().getObject().getSpecifiers(), to))) {
          return a;
        }
      }
    }
   ...
   * @return
   */
  private static boolean specifiersContain(ArrayList<Specifier> specifiers, SpecifiedElement to) {
    for (Specifier spec : specifiers) {
      if (spec.getObject() != null) {
        if (spec.getObject().equals(to)
            || specifiersContain(spec.getObject().getSpecifiers(), to)) {
          return true;
        }
      }
    }
  ...
  /**
   * returns the object of the keySet which has the highest associated count if the map is empty,
   ...
   * @return
   */
  public static List<TypedDependency> filterByIndex(
      Action verb, List<TypedDependency> dep, boolean smaller) {
    List<TypedDependency> _result = new ArrayList<TypedDependency>();
    for (TypedDependency td : dep) {
   ...
   * @return
   */
  public static List<TypedDependency> filterByGov(
      TreeGraphNode gov, Collection<TypedDependency> dep) {
    List<TypedDependency> _result = new ArrayList<TypedDependency>();
    for (TypedDependency td : dep) {
