...
public class AnalyzedSentence {
  ...
  private final boolean f_ignoreNPSubSentences =
      "1"
          .equals(
              Configuration.getInstance().getProperty(Constants.CONF_GENERATE_IGNORE_SBAR_ON_NP));

  private static final Logger logger = LoggerFactory.getLogger(AnalyzedSentence.class);
  ...
  private void analyzseSentence(Tree _mainSentence, Collection<TypedDependency> dependencies) {
    ...
    if (_sCount == 0) {
      ...
      if (SearchUtils.findDependency(ListUtils.getList("nsubj", "agent", "nsubjpass", "dobj"), _fd)
              .size()
          > 0) {
      ...
      for (Tree sentP : _sentencesParts) {
        ...
        List<TypedDependency> _dependenciesFiltered =
            SearchUtils.filter(f_dependencies, _startIndex, _endIndex);
  ...
  private int determineSubSentenceCount(Tree sentence) {
    ...
    // ignore it if the root if the only node (e.g. for relaitve sentence
    // TextToWorldModel.processing)
   ...
   * @return
   */
  private List<TypedDependency> filterDependencies(
      Tree sentP, Collection<TypedDependency> dependencies) {
    List<Tree> _partSentence = sentP.getLeaves();
  ...
  private void extractElements(Tree sentence, Collection<TypedDependency> dependencies) {
    ...
    for (Action verb : _verbs) {
      List<ExtractedObject> _objects =
          determineObject(sentence, _active, verb, dependencies, _active);
    ...
    for (Action a : _finalActions) {
      f_actions.add(a);
      if (a.getXcomp() != null) {
        if (a.getXcomp().getActorFrom() != null) f_world.addActor(a.getXcomp().getActorFrom());
  ...
  /**
   * This method checks, if the actor is a personal pronoun and replaces the personal pronoun with
   * the previous actor.
   *
   * @param actors
   */
  ...
  /**
   * if one of the specifiers contains a subject (in passive sentences) it is removed to make the
   * verb leaner
   *
   * @param verb
   * @param actors
  ...
  private void filterVerb(Action verb, List<Actor> actors, List<ExtractedObject> objects) {
    ...
    for (SpecifiedElement a : _toCheck) {
      for (Specifier sp : _specToCheck) {
        if (sp.getWordIndex()
            == a.getWordIndex() /*checkContainment(sp.getPhrase(),a.getName())*/) {
  ...
  private boolean isActive(Tree sentence, Collection<TypedDependency> dependencies) {
    List<TypedDependency> _nsubj =
        SearchUtils.findDependency(ListUtils.getList("nsubj", "csubj", "dobj"), dependencies);
    excludeRelativeClauses(sentence, _nsubj);
    if (_nsubj.size() > 0) {
      return true;
    }
    List<TypedDependency> _nsubjpass =
        SearchUtils.findDependency(
            ListUtils.getList("nsubjpass", "csubjpass", "agent"), dependencies);
  ...
  /**
   * extracts the subjects (actors) of the activity can be null (in case of a passive sentence) or a
   * relative pronoun which then has to be resolved
   *
   * @param sentence
   * @param dependencies
   * @param active
   */
  private List<Actor> determineSubjects(
      Tree sentence, Collection<TypedDependency> dependencies, boolean active) {
    ...
    if (_mainActor != null) {
      Actor _actor =
          ElementsBuilder.createActor(f_sentence, f_fullSentence, _mainActor, dependencies);
      _actor.setSubjectRole(true);
      _actor.setPassive(!active);
      _result.add(_actor);
      ArrayList<SpecifiedElement> _list =
          checkConjunctions(dependencies, _actor, true, true, active);
  ...
  private ArrayList<SpecifiedElement> checkConjunctions(
      Collection<TypedDependency> dependencies,
      SpecifiedElement current,
      boolean object,
      boolean active) {
  ...
  /**
   * checks if our main subject is connected to someone else via a conjunction (or, and) because
   * then those are also actors (... has to be checked by the CEO -and- the COO)
   *
   * @param dependencies
   * @param current
   * @param object
    ...
    if (_conj.size() > 0) {
      ...
      for (TypedDependency td : _conj) {
        boolean _xcompHit =
            (a != null
                && a.getXcomp() != null
                && a.getXcomp().getVerb().contains(td.gov().value()));
        if ((td.gov().value().equalsIgnoreCase(current.getName())
          ...
          if (object) {
            if (actor) {
              _newEle =
                  ElementsBuilder.createActor(f_sentence, f_fullSentence, _otherNode, dependencies);
            } else {
              _newEle =
                  ElementsBuilder.createObject(
                      f_sentence, f_fullSentence, _otherNode, dependencies);
            ...
            if (_xcompHit) {
              ...
              ((Action) _newEle)
                  .setXcomp(
                      ElementsBuilder.createAction(
                          f_sentence, f_fullSentence, _otherNode, dependencies, true));
            } else {
              _newEle =
                  ElementsBuilder.createAction(
                      f_sentence, f_fullSentence, _otherNode, dependencies, active);
            }
          }
   ...
   * @return
   */
  private List<Action> determineVerb(
      Tree sentence, boolean active, Collection<TypedDependency> dependencies) {
      ...
      if (_nsubj.size() == 0) {
        ...
        if (_dobj.size() >= 1) {
          _mainPredicate =
              _dobj
                  .get(0)
                  .gov(); // here several are possible "leave the house and close the window" ->
          // conj will find the rest
        }
    ...
    if (_mainPredicate == null) {
      ...
      Action _a =
          ElementsBuilder.createAction(
              f_sentence, f_fullSentence, _mainPredicate, dependencies, active);
   ...
   * @param dependencies
   */
  private void checkForSubSentences(
      Tree head, Collection<TypedDependency> dependencies, SpecifiedElement object, boolean isNP) {
    ...
    if (f_sentenceTags.contains(head.value())) {
      ...
      if (mainAction == null
          || mainAction.getXcomp() == null
          || ((_start > mainAction.getXcomp().getWordIndex())
              || (_end < mainAction.getXcomp().getWordIndex()))) {
        analyzseSentence(head, filterDependencies(head, dependencies));
      }
    } else {
      if (head.value().equals("PP")
          || head.value().equals("VP")
          || (isNP && head.value().equals("NP"))) {
   ...
   * @param _active
   */
  private List<ExtractedObject> determineObject(
      Tree sentence,
      boolean _active,
      Action verb,
      Collection<TypedDependency> dependencies,
    ...
    if (!_active) {
      ...
      if (_nsubjpass.size() == 0) {
        ...
        ExtractedObject _obj =
            ElementsBuilder.createObject(f_sentence, f_fullSentence, _object, dependencies);
        _obj.setSubjectRole(
            true); // although it is an object it is the syntactic subject of the sentence
        _result.add(_obj);
    ...
    if (_result.size() > 0) {
      ...
      for (SpecifiedElement sp : _conj) {
        if (sp instanceof ExtractedObject) _result.add((ExtractedObject) sp);
      }
    }
    return _result;
  }

  private void determineObjectFromDOBJ(
      Action verb, Collection<TypedDependency> dependencies, List<ExtractedObject> _result) {
    List<TypedDependency> _dobj =
        SearchUtils.findDependency(ListUtils.getList("dobj"), dependencies);
    List<TypedDependency> _myDobj = SearchUtils.filterByGov(verb, _dobj);
    if (_myDobj.size() == 0) {
      if (!(verb.getXcomp() != null && verb.getXcomp().getObject() != null)) {
        // if we already have an xcomp object, we do not have to check ands (incident management -
        // BPMN standard)
      ...
      if (_myPrep.size() == 0) {
        ...
        if (_cop.size() == 0) {
          ...
          if (_object
              ...
              .parent()
              .value()
              .equals("NP")) { // only if it is directly part of a noun phrase
            ExtractedObject _obj =
                ElementsBuilder.createObject(f_sentence, f_fullSentence, _object, dependencies);
            _result.add(_obj);
            checkNPForSubsentence(_object, dependencies, _obj);
  ...
  /** */
  private void checkNPForSubsentence(
      TreeGraphNode tgNode, Collection<TypedDependency> dependencies, ExtractedObject obj) {
  ...
  private void excludeRelativeClauses(Tree sentence, List<TypedDependency> list) {
    for (int i = 0; i < list.size(); i++) {
      ...
      while (!(_dep = (TreeGraphNode) _dep.parent()).value().equals("ROOT")) {
        ...
        if ((_dep.parent().value().equals("SBAR")
                || _dep.parent().value().equals("S")
                || _dep.parent().value().equals("PRN"))
            && (!_dep.parent()
                .parent()
                .value()
                .equals("SBAR"))) { // hack for relative clause TextToWorldModel.processing
          list.remove(i);
          i--;
          break;
        }
