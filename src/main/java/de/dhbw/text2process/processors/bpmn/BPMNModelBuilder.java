...
import de.dhbw.text2process.models.bpmn.MessageIntermediateEvent;
...
import de.dhbw.text2process.models.bpmn.TimerIntermediateEvent;
...
import de.dhbw.text2process.processors.meta.ProcessModelBuilder;
...
public class BPMNModelBuilder extends ProcessModelBuilder {
  ...
  private final boolean REMOVE_LOW_ENTROPY_NODES =
      "1".equals(f_config.getProperty(Constants.CONF_GENERATE_REMOVE_LOW_ENT_NODES));
  private final boolean HIGHLIGHT_LOW_ENTROPY =
      "1".equals(f_config.getProperty(Constants.CONF_GENERATE_HIGHLIGHT_META_ACTIONS));
  // Labeling
  private final boolean ADD_UNKNOWN_PHRASETYPES =
      "1".equals(f_config.getProperty(Constants.CONF_GENERATE_ADD_UNKNOWN_PT));
  private final int MAX_NAME_DEPTH = 3;
  // Model in General
  private final boolean BUILD_BLACK_BOX_POOL_COMMUNICATION =
      "1".equals(f_config.getProperty(Constants.CONF_GENERATE_BB_POOLS));
  private final boolean BUILD_DATA_OBJECTS =
      ...
      if (node instanceof Gateway) {
        ...
        for (ProcessNode _succ : _succs) {
          if (_succ
              .getClass()
              .getSimpleName()
              .equals("IntermediateEvent")) { // only simple intermediate events
            ...
            for (Specifier spec : _specs) {
              if (SearchUtils.startsWithAny(spec.getPhrase(), Constants.f_conditionIndicators)
                  && !Constants.f_conditionIndicators.contains(
                      spec.getPhrase())) { // it should only be the start of a phrase, not the whole
                // phrase!
    ...
    if (WordNetWrapper.isWeakAction(a) && canBeTransformed(a)) {
      if (a.getActorFrom() != null
        ...
        _b.append(transformToAction(a.getActorFrom()));
          ...
          && ((a.getObject() instanceof Resource) || !((Actor) a.getObject()).isUnreal())) {
      ...
      if (!_weak) {
        ...
        if (REMOVE_LOW_ENTROPY_NODES
            && (a.getActorFrom() == null || a.getActorFrom().isMetaActor())
            && a.getXcomp() == null) {
          return "Dummy";
        } else {
      ...
      if (!(a.getObject() == null)) { // otherwise addSpecifiers already did the work!
        for (Specifier sp : a.getSpecifiers(SpecifierType.PP)) {
          if ((sp.getName().startsWith("to")
                  || sp.getName().startsWith("in")
                  || sp.getName().startsWith("about"))
  ...
  protected boolean considerPhrase(Specifier spec) {
    if (spec.getPhraseType() == PhraseType.PERIPHERAL
        || spec.getPhraseType() == PhraseType.EXTRA_THEMATIC) {
      return false;
    } else {
    ...
    if (_obj != null && _obj instanceof Actor && !((Actor) _obj).isMetaActor()) {
      ...
      if (_sender != null && !f_NameToPool.containsKey(getName(_sender.getObject(), false))) {
        ...
        if (f_NameToPool.containsKey(_name)) {
          ...
          && !f_NameToPool.containsKey(getName(_receiver.getObject(), false))) {
