...
public class PetrinetBuilder {
  ...
  private void createJoin(Flow f) {
    ...
    petriNet.add(
        elementBuilder.createArc(target.getID(), t.getID(), getOriginID(f.getSingleObject())));
    petriNet.add(
        elementBuilder.createArc(
            t.getID(), singletarget.getID(), getOriginID(f.getSingleObject())));
  ...
  private Transition createTransition(Action a, boolean isGateway) {
    ...
    // Transition Text
    Transition t =
        elementBuilder.createTransition(a.getFinalLabel(), hasRole, isGateway, getOriginID(a));

    // Resource Text
    // TODO Refactor
    if (a.getObject() != null) t.setResourceName(a.getObject().getName());
    ...
    if (a.getActorFrom() != null) {
      if (a.getActorFrom().getReference() != null) {
        if (a.getActorFrom().getReference().getSpecifiers(SpecifierType.NN).size() > 0) {
          t.setOrganizationalUnitName(
              a.getActorFrom().getReference().getSpecifiers(SpecifierType.NN).get(0).getName());
        } else {
        ...
        if (a.getActorFrom().getSpecifiers(SpecifierType.NN).size() > 0) {
          t.setOrganizationalUnitName(
              a.getActorFrom().getSpecifiers(SpecifierType.NN).get(0).getName());
        }
  ...
  /**
   * This method creates the xml representation of the WoPeD-Graph. Each Object is wrapped in a
   * xml-String and added to the petriNet object.
   *
   * @param f Flow Object
   */
  private void createSequence(Flow f) {

    // check refers (apparently) to a bug during WorldModel Creation: Sequences among Equal Actions
    // are created -> Inconsistency -> ignore
    if (!getOriginID(f.getMultipleObjects().get(0)).equals(getOriginID(f.getSingleObject()))) {
      ...
      if (!initiallyFound) {
        ...
        petriNet.add(
            elementBuilder.createArc(t0.getID(), p1.getID(), getOriginID(f.getSingleObject())));
      }
      ...
      Arc a1 =
          elementBuilder.createArc(
              p1.getID(), t1.getID(), getOriginID(f.getMultipleObjects().get(0)));
      Arc a2 =
          elementBuilder.createArc(
              t1.getID(), p2.getID(), getOriginID(f.getMultipleObjects().get(0)));
      petriNet.add(a1);
      petriNet.add(a2);
    }
  }
  ...
  private void createSplit(Flow f) {
    ...
    } else if (f.getType().equals(FlowType.choice)
        || f.getType().equals(FlowType.multiChoice)
        || f.getType().equals(FlowType.iteration)) {
      if (label.equals("")) label = "choice";
    ...
    while (i.hasNext()) {
      ...
      if (ingoingArcs.size() == 1) {
        ...
        Iterator<Arc> nsI =
            petriNet
                .getAllReferencingArcsForElement(
                    dummyTransition.getID(), PetriNet.REFERENCE_DIRECTION.ingoing)
                .iterator();
        while (nsI.hasNext()) {
          Arc a = nsI.next();
          ...
          while (newTargetIterator.hasNext()) {
            petriNet.add(
                elementBuilder.createArc(a.getSource(), newTargetIterator.next().getID(), ""));
          }
          petriNet.removePetrinetElementByID(a.getID());
        }
